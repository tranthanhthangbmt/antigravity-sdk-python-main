...
# Kế hoạch tự động dịch Chương 2 (Chủ đề Chương 2)

Dựa trên yêu cầu và phương pháp đã áp dụng, tôi sẽ tự động trích xuất văn bản từ `Chapter_02.pdf` và tiến hành dịch theo từng phần (chunk) để đảm bảo chất lượng, giữ nguyên cấu trúc, hình ảnh, code và công thức toán học. Đích đến cuối cùng là xuất ra sách LaTeX và biên dịch thành PDF tương tự như project `ML_new1_2026`.

Với Chương 2, khối lượng văn bản khá lớn và chứa nhiều mã nguồn Java, hình ảnh minh họa cũng như bài tập. Việc chia nhỏ tài liệu một cách logic là rất cần thiết để quá trình dịch tự động hoạt động mượt mà.

## Phân chia tài liệu (Tree of Thought)
Văn bản PDF Chương 2 sẽ được tự động phân chia thành các phần logic dựa trên độ dài và cấu trúc tiêu đề trong quá trình trích xuất để đảm bảo không làm quá tải AI:

- **Phần 1**: 2.1 Basic Data Concepts
- **Phần 2**: 2.2 Variables
- **Phần 3**: 2.3 The for Loop
- **Phần 4**: 2.4 Managing Complexity
- **Phần 5**: 2.5 Case Study: Hourglass Figure
...
4. **Hình ảnh (Images)**: Dùng công cụ `pymupdf` để trích xuất hình ảnh từ PDF gốc và lưu vào thư mục `images/chapter_02/`. Đảm bảo các link hình ảnh trong file markdown trỏ đúng đường dẫn.
5. **Công thức toán học (Math)**: Giữ nguyên các định dạng công thức toán học (`$...$` hoặc `$$...$$`).

## Quy trình xuất bản (Publishing Pipeline)
1. **Trích xuất & Dịch**: Sử dụng tập lệnh Python trích xuất text và ảnh theo từng khoảng trang cụ thể. AI sẽ dịch thô và lưu kết quả ra file Markdown `chapters/chapter_02_partY.md`.
2. **Tiền xử lý (Preprocessing)**: Chạy `build_latex.py` để:
   - Gom các phần markdown lại thành `chapter_02_all.md`.
   - Lọc bỏ ảnh rác `< 10KB`.
...
3. **Chuyển đổi LaTeX & Biên dịch**: 
   - Dùng Pandoc sinh `chapter_02.tex`.
...
> Nếu bạn đồng ý với kế hoạch của Chương 2, tôi sẽ kích hoạt pipeline để máy chạy ngay lập tức.
